...
func RemoteDatabasePlugin(engineKind string) (string, error) {
	...
	default:
		return "", fmt.Errorf("database engine %q is not supported through the OpenBao spoke relay; supported: postgres, mysql, mariadb, redis, valkey, rabbitmq", engineKind)
	}
}
