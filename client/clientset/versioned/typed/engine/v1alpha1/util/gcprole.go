...
func CreateOrPatchGCPRole(
	...
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating GCPRole %s/%s.", meta.Namespace, meta.Name)
...
func PatchGCPRoleObject(
	...
	klog.V(3).Infof("Patching GCPRole %s/%s with %s.", cur.Namespace, cur.Name, string(patch))
...
func TryUpdateGCPRole(
	...
	err = wait.PollUntilContextTimeout(context.Background(), kutil.RetryInterval, kutil.RetryTimeout, true, func(ctx context.Context) (bool, error) {
		...
		klog.Errorf("Attempt %d failed to update GCPRole %s/%s due to %v.", attempt, cur.Namespace, cur.Name, e2)
		return false, nil
	})
