...
func CreateOrPatchMySQLRole(
	...
	if kerr.IsNotFound(err) {
		klog.V(3).Infof("Creating MySQLRole %s/%s.", meta.Namespace, meta.Name)
...
func PatchMySQLRoleObject(
	...
	klog.V(3).Infof("Patching MySQLRole %s/%s with %s.", cur.Namespace, cur.Name, string(patch))
...
func TryUpdateMySQLRole(
	...
	err = wait.PollUntilContextTimeout(context.Background(), kutil.RetryInterval, kutil.RetryTimeout, true, func(ctx context.Context) (bool, error) {
		...
		klog.Errorf("Attempt %d failed to update MySQLRole %s/%s due to %v.", attempt, cur.Namespace, cur.Name, e2)
		return false, nil
	})
...
func UpdateMySQLRoleStatus(
	...
	if err != nil {
		err = fmt.Errorf("failed to update status of MySQLRole %s/%s after %d attempts due to %v", meta.Namespace, meta.Name, attempt, err)
	}
	return
}
