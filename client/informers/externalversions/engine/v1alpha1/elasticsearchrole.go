...
// ElasticsearchRoleInformer provides access to a shared informer and lister for
// ElasticsearchRoles.
...
// NewElasticsearchRoleInformer constructs a new informer for ElasticsearchRole type.
...
func NewElasticsearchRoleInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredElasticsearchRoleInformer(client, namespace, resyncPeriod, indexers, nil)
}

// NewFilteredElasticsearchRoleInformer constructs a new informer for ElasticsearchRole type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
...
func (f *elasticsearchRoleInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredElasticsearchRoleInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}
