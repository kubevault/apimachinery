...
type VaultServerLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.VaultServer, err error)
...
// List lists all VaultServers in the indexer.
func (s *vaultServerLister) List(selector labels.Selector) (ret []*v1alpha2.VaultServer, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.VaultServer))
	})
	return ret, err
}
...
type VaultServerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.VaultServer, error)
...
// List lists all VaultServers in the indexer for a given namespace.
func (s vaultServerNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.VaultServer, err error) {
...
// Get retrieves the VaultServer from the indexer for a given namespace and name.
func (s vaultServerNamespaceLister) Get(name string) (*v1alpha2.VaultServer, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("vaultserver"), name)
	}
	return obj.(*v1alpha2.VaultServer), nil
}
