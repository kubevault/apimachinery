...
# invocation, using the generator binaries from k8s.io/code-generator's
# kube_codegen.sh toolchain
...
# generation footprint -- every apis/<group>/<version> package carries a
# +k8s:defaulter-gen marker, but defaulter-gen has never actually been run
# here (no zz_generated.defaults.go exists in the repo). So the generator
# binaries are invoked directly instead, with the exact same
# $(API_GROUPS)-driven scope the old Makefile targets used, to keep
...
THIS_PKG="kubevault.dev/apimachinery"
...
# "kubevault:v1alpha1,v1alpha2" -> kubevault/v1alpha1 kubevault/v1alpha2.
group_versions=()
...
# Conversion helpers between apis/kubevault/v1alpha1 and apis/kubevault/v1alpha2,
# the same scope the old ad hoc conversion-gen invocation used. apis/kubevault/v1alpha1's
# doc.go declares v1alpha2 as its +k8s:conversion-gen peer package, so
# conversion-gen discovers it without needing --extra-peer-dirs.
echo "Generating conversion code for apis/kubevault/v1alpha1"
"${GOBIN}/conversion-gen" \
    --go-header-file "${BOILERPLATE}" \
    --output-file zz_generated.conversion.go \
    "${THIS_PKG}/apis/kubevault/v1alpha1"
